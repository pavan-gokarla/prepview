...
import { getUser, signInGoogle } from '@/lib/actions/actions';
...
export function Form({ signIn }: { signIn: boolean }) {
    ...
    const userAction = (formData: FormData) => {
    ...
    return (<div className=' text-white w-[100%] ' >
        <div className='flex flex-col gap-1.5' >
            ...
            <p className='text-[var(--noble--black--300)]' >{signIn ? "Log in" : "Sign Up"} to Prepview to start preparing</p>
        </div>
        <form action={userAction} id="inputs" className="mt-[1.5rem] flex flex-col justify-around gap-5">
            {!signIn && (
                <Input name="name" type="text" placeholder="Your name" className="capitalize">
                    <User className="absolute top-[30%] right-[3%]" size={16} />
                </Input>
            )}

            <Input type="email" name="email" placeholder="Email" className="capitalize">
                <Mail className="absolute top-[30%] right-[3%]" size={16} />
            </Input>

            <Input name="password" type="password" placeholder="Password" className="capitalize">
                <Lock className="absolute top-[30%] right-[3%]" size={16} />
            </Input>

            {!signIn && (
                <Input name="confirm-password" type="password" placeholder="Confirm password" className="capitalize">
                    <LockOpen className="absolute top-[30%] right-[3%]" size={16} />
                </Input>
            )}
        </form>
        <div id='forget' className='mt-0.5 flex flex-col items-end gap-5 ' >
            <Button variant='link' className='text-[16px] cursor-pointer text-[var(--noble--black--300)] ' >Forgot Password</Button>
        </div>
        <div className="sign-in  flex  mt-[0.1rem] gap-3 " >
            <Button variant="default" className=' flex-grow bg-[var(--stem--green--500)] text-[var(--day--blue--900)] hover:bg-[var(--stem--green--400)]  ' >{signIn ? "Sign In" : "Sign Up"}</Button>
        </div>
